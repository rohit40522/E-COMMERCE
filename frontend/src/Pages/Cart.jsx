          ...
          cartData.map((item, index)  =>{
            ...
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                ...
                <img className='w-4 mr-4 sm:mr-5 cursor-pointer ' src={assets.bin_icon} onClick={() => updateQuantity(item._id, item.size, 0)} alt="" />
              </div>
            )
          })
        }
      ...
      <div className='flex justify-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            <button onClick={()=> navigate('/place-order')} className='bg-black text-white text-sm my-8 px-8 py-3 hover:cursor-pointer'> Proceed to check out</button>
          </div>
        </div>
      </div>
